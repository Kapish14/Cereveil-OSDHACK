...
```ts
{
  ...
  activeScreenSafety: {
    scamText: {
      enabled: boolean,
      monitoredPackageNames: Array<string>,
      sensitivity: "lower" | "standard" | "higher",
    },
    nsfwScreen: {
      enabled: boolean,
      monitoredPackageNames: Array<string>,
...
- Schema v3 adds independent Scam Text Detection and NSFW Screen Detection sections. V1 and v2 snapshots retain the safe meaning of both detectors disabled, no Monitored Apps, and `standard` sensitivities; Guardian Mode cannot change either section until the Active Enrollment reports schema v3 support.
- Each detector requires at least one explicitly selected package when enabled. Disabling one preserves its own `monitoredPackageNames` and sensitivity, while removing every selected package leaves that detector disabled. The same package may be selected for either detector or both.
- Scam Text Detection is available on Android 8 and newer; NSFW Screen Detection policy changes require an enrolled Child Device running Android 11 or newer.
...
by_household_id
by_active_enrollment_id_and_incident_id
by_expires_at
```
...
- Retain for one week.
- Accept idempotent retries by Child-generated `incidentId`.
- Create an immediate Guardian Notice only when the alert arrives within five minutes of `occurredAt` and its detection type is outside the fixed, non-sliding two-minute Guardian Notification Cooldown.
